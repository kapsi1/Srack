...
export function Sidebar({
	...
	return (
		<div
			className={`flex h-screen bg-[#19171d] transition-all duration-300 ease-in-out ${isCollapsed ? 'w-0' : 'w-[260px]'} overflow-hidden`}
		>
			...
			<div className="w-[260px] flex flex-col text-white">
				...
				<div className="flex-1 overflow-y-auto py-2">
					...
					<div className="px-2 mb-4">
						<SidebarItem icon={MessageSquare} label="Threads" to="/threads" active={location.pathname === '/threads'} />
						<SidebarItem
							icon={AtSign}
						...
						<SidebarItem
							...
							label="Saved items"
							to="/saved-items"
							active={location.pathname === '/saved-items'}
						/>
					...
					<div>
						...
						<div className="mt-1">
							{directMessages.map((dm: DirectMessage) => {
								const isActive =
									activeChannel?.type === 'DM' && activeChannel.members?.some((m: User) => m.id === dm.id);
								return (
									<Link
										key={dm.id}
										to={`/user/${dm.userName}`}
				...
				<div className="h-12 px-2 border-t border-white/10 flex items-center justify-between">
					...
					<div className="p-2 hover:bg-gray-800 transition-colors rounded" title="Logout">
...
function SidebarItem({
	...
	if (to) {
		return (
			<Link
				to={to}
				className={`w-full px-2 py-1 flex items-center gap-1.5 hover:bg-white/10 transition-colors rounded cursor-default ${active ? 'bg-[#1164a3]' : ''}`}
			>
				{content}
			</Link>
		);
